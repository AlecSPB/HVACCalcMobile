...
public class AirDuctCalculator {
    ...
    AirDuctCalculator(float flowRate, float width, float height, float temperature, float roughness){
    ...
    private void printVelocity(){
        System.out.println("Скорость воздуха в воздуховоде: " + this.velocity + " м/с");
    }

    private void printPressureDrop(){
        System.out.println("Падение напора в воздуховоде: " + this.pressureDrop + " Па/м");
    }
    ...
    private double getViscosity(){
        ...
        for (int i = 1; i < temp.length; i++) {
            if (t <= this.temperature && this.temperature < temp[i]){
                viscosity = this.viscosityTable.get(temp[i - 1]) + ((this.temperature - temp[i - 1]) / (temp[i] - temp[i - 1]))
                        * (this.viscosityTable.get(temp[i]) - this.viscosityTable.get(temp[i - 1]));
                break;
            }
            else t = temp[i];
        }
    ...
    private double calculateReynoldsNumber(){
        return (this.density * this.velocity * this.diameter) / getViscosity();
    }
